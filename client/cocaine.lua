...
local function SpawnSingleProp(zoneIndex, propIndex)
    ...
    local propModel = CocaineConfig.HarvestProp
    ...
    exports.ox_target:addLocalEntity(prop, {
        {
            name = 'cocaine_harvest_' .. zoneIndex .. '_' .. propIndex,
            label = zone.label,
            icon = 'fa-solid fa-leaf',
            onSelect = function()
                HarvestCocaineProp(zoneIndex, propIndex)
            end
        }
    })
    
    if Config.Debug then
        print(string.format("[Cocaine] Spawned prop %d in zone %d at %s", propIndex, zoneIndex, zone.name))
    end
...
local function DeleteProp(zoneIndex, propIndex)
    ...
    if prop and DoesEntityExist(prop) then
        exports.ox_target:removeLocalEntity(prop, 'cocaine_harvest_' .. zoneIndex .. '_' .. propIndex)
        DeleteEntity(prop)
        harvestProps[zoneIndex][propIndex] = nil
        
        if Config.Debug then
            print(string.format("[Cocaine] Deleted prop %d in zone %d", propIndex, zoneIndex))
        end
    end
end

local function CreateHarvestProps()
    for i, zone in ipairs(CocaineConfig.HarvestZones) do
...
local function CreateProcessingZones()
    for i, location in ipairs(CocaineConfig.ProcessingLocations) do
        local zoneId = exports.ox_target:addSphereZone({
            ...
            options = {
                {
                    name = 'cocaine_process_' .. i,
                    label = 'Process Cocaine',
                    icon = 'fa-solid fa-flask',
                    items = { "cocaine_leaf", "baggies" },
                    onSelect = function()
                        StartCocaineProcessing(i)
                    end
                }
            }
        })
        ...
        if Config.Debug then
            print(string.format("[Cocaine] Created processing zone at %s: %.2f, %.2f, %.2f", location.name, location.coords.x, location.coords.y, location.coords.z))
        end
    end
end

function HarvestCocaineProp(zoneIndex, propIndex)
    ...
    if lib.progressCircle({
        ...
        local success, result = lib.callback.await('drugs:server:harvestCocaine', false, zoneIndex)
        
        if success then
            lib.notify({ title = 'Harvested', description = 'Collected ' .. result .. ' coca leaves', type = 'success' })
...
function StartCocaineProcessing(locationIndex)
    if isProcessing then
        lib.notify({ title = 'Cocaine', description = 'Already processing', type = 'error' })
        return
    end
    
    isProcessing = true
    
    local success, result = lib.callback.await('drugs:server:processCocaine', false, locationIndex)
    
    if not success then
        lib.notify({ title = 'Cocaine Processing', description = result, type = 'error' })
        isProcessing = false
        return
    end
    
    local processingTime = CocaineConfig.ProcessingTime
    ...
    if lib.progressCircle({
        ...
        lib.notify({ title = 'Cocaine', description = 'Processed ' .. result .. ' bags of cocaine!', type = 'success' })
    else
        lib.notify({ title = 'Cocaine', description = 'Processing cancelled', type = 'error' })
    end
